 ...
 */
#define _XENUM5_TT_ITERPOS_LEVEL(ITERPOS)		BOOST_PP_SEQ_ELEM(0, ITERPOS)
 ...
 */
#define _XENUM5_TT_ITERPOS_INDEXPATH(ITERPOS)		BOOST_PP_SEQ_ELEM(1, ITERPOS)
 ...
 */
#define _XENUM5_TT_ITERPOS_NEXTCHILD(ITERPOS)		BOOST_PP_SEQ_ELEM(2, ITERPOS)
 ...
 */
#define _XENUM5_TT_ITERPOS_CHILDCOUNT(ITERPOS)		BOOST_PP_SEQ_ELEM(3, ITERPOS)
...
/*	iv=_XENUM5_TT_ITERPOS_IS_VALUE(ITERPOS) */						\
	lv=_XENUM5_TT_ITERPOS_LEVEL(ITERPOS)							\
	ip(_XENUM5_GET_SEQ_OR_EMPTY_SIZE(_XENUM5_TT_ITERPOS_INDEXPATH(ITERPOS)))=_XENUM5_TT_ITERPOS_DUMP_INDEXPATH(_XENUM5_TT_ITERPOS_INDEXPATH(ITERPOS))	\
	nc=_XENUM5_TT_ITERPOS_NEXTCHILD(ITERPOS)						\
	cc=_XENUM5_TT_ITERPOS_CHILDCOUNT(ITERPOS)						\
 ...
 * @return Path through the data tree to the current node, as a seq of indexes.
 */
#define _XENUM5_TT_STACKENTRY_INDEXPATH(STACKENTRY)						\
	_XENUM5_TT_ITERPOS_INDEXPATH(_XENUM5_TT_STACKENTRY_ITERPOS(STACKENTRY))

/**
 * @return Object describing current position in the data tree.
 */
#define _XENUM5_TT_STACKENTRY_ITERPOS(STACKENTRY) BOOST_PP_SEQ_ELEM(0, STACKENTRY)
 ...
 */
#define _XENUM5_TT_STACKENTRY_NODE(STACKENTRY) BOOST_PP_SEQ_ELEM(1, STACKENTRY)
...
#define _XENUM5_TT_STACKENTRY_DUMP(STACKENTRY)							\
	iterpos:{_XENUM5_TT_ITERPOS_DUMP(_XENUM5_TT_STACKENTRY_ITERPOS(STACKENTRY))}		\
	node=[_XENUM5_TT_STACKENTRY_NODE(STACKENTRY)]
 ...
 */
#define _XENUM5_TT_STATE_STACK(ITERSTATE)							\
	BOOST_PP_SEQ_ELEM(0, ITERSTATE)

/**
 * @return Head of iteration stack.
 */
#define _XENUM5_TT_STATE_STACKHEAD(ITERSTATE)							\
	BOOST_PP_SEQ_HEAD(_XENUM5_TT_STATE_STACK(ITERSTATE))
 ...
 */
#define _XENUM5_TT_STATE_CALLBACK(ITERSTATE)							\
 ...
 */
#define _XENUM5_TT_STATE_FILTER(ITERSTATE)							\
 ...
 */
#define _XENUM5_TT_STATE_CTXT(ITERSTATE)							\
	BOOST_PP_SEQ_ELEM(2, ITERSTATE)

/**
 * @return Caller data store.
 */
#define _XENUM5_TT_STATE_CDATA(ITERSTATE)							\
...
#define _XENUM5_TT_FILTER_LEAF(ITERPOS, NODE, CTXT, ...)					\
	BOOST_PP_NOT(_XENUM5_TT_ITERPOS_LEVEL(ITERPOS))
...
#define _XENUM5_TT_CMP(N, ITERSTATE)								\
	BOOST_PP_NOT(BOOST_PP_IS_EMPTY(_XENUM5_TT_STATE_STACK(ITERSTATE)))
...
#define _XENUM5_TT_DEPTH_INC(ITERSTATE)								\
	_XENUM5_TT_DEPTH_INC_I1(								\
		ITERSTATE,									\
		_XENUM5_TT_STATE_STACK(ITERSTATE),						\
		_XENUM5_TT_STATE_STACKHEAD(ITERSTATE)						\
	)											\
...
#define _XENUM5_TT_DEPTH_INC_I1(ITERSTATE, STACK, HEAD)						\
	BOOST_PP_CAT(										\
		...
		_XENUM5_TT_DEPTH_INC_STACKENTRY_DONE(_XENUM5_TT_STACKENTRY_ITERPOS(HEAD))	\
	) (ITERSTATE, STACK, HEAD)
/*
_TT_DEPTH_INC_I1: head=_XENUM5_TT_STACKENTRY_DUMP(HEAD) done=_XENUM5_TT_DEPTH_INC_STACKENTRY_DONE(_XENUM5_TT_STACKENTRY_ITERPOS(HEAD)) _XENUM5_NWLN \
*/
...
#define _XENUM5_TT_DEPTH_INC_STACKENTRY_DONE(ITERPOS)						\
	BOOST_PP_OR(										\
		BOOST_PP_NOT(_XENUM5_TT_ITERPOS_LEVEL(ITERPOS)),				\
		BOOST_PP_EQUAL(									\
			_XENUM5_TT_ITERPOS_NEXTCHILD(ITERPOS),					\
...
#define _XENUM5_TT_DEPTH_INC_I1_0(ITERSTATE, STACK, HEAD)					\
	_XENUM5_TT_DEPTH_INC_NEXT_CHILD(							\
		ITERSTATE,									\
		STACK,										\
		_XENUM5_TT_STACKENTRY_NODE(HEAD),						\
		_XENUM5_TT_STACKENTRY_ITERPOS(HEAD)						\
	)
...
#define _XENUM5_TT_DEPTH_GEN_EXEC(N, ITERSTATE)							\
	_XENUM5_TT_DEPTH_GEN_EXEC_I1(ITERSTATE, _XENUM5_TT_STATE_STACKHEAD(ITERSTATE))
/*
_TT_DEPTH_GEN_EXEC: n=N callback=_XENUM5_TT_STATE_CALLBACK(ITERSTATE) filter=_XENUM5_TT_STATE_FILTER(ITERSTATE) _XENUM5_NWLN \
_TT_DEPTH_GEN_EXEC: n=N stack=BOOST_PP_SEQ_SIZE(_XENUM5_TT_STATE_STACK(ITERSTATE)) \
*/
...
#define _XENUM5_TT_DEPTH_GEN_EXEC_I1(ITERSTATE, HEAD)						\
	...
	(											\
		...
		_XENUM5_TT_STACKENTRY_ITERPOS(HEAD),						\
		_XENUM5_TT_STACKENTRY_NODE(HEAD),						\
		_XENUM5_TT_STATE_CTXT(ITERSTATE)						\
	)
...
#define _XENUM5_TT_DEPTH_GEN_EXEC_I2(ITERSTATE, ITERPOS, NODE, CTXT)				\
	BOOST_PP_CAT(										\
		...
		_XENUM5_TT_STATE_FILTER(ITERSTATE) (ITERPOS, NODE, CTXT)			\
...
#define _XENUM5_TT_DEPTH_GEN_EXEC_1(ITERSTATE, ITERPOS, NODE, CTXT)				\
	_XENUM5_TT_STATE_CALLBACK(ITERSTATE) (ITERPOS, NODE, CTXT)
...
#define _XENUM5_TT_DBG_NODE_NAME(ITERSTATE)							\
	[_XENUM5_CTXT_IDENT(_XENUM5_TT_STATE_CTXT(ITERSTATE)					\
	),_XENUM5_PDEF_NAME(									\
		_XENUM5_CTXT_PDEF(								\
			_XENUM5_TT_STATE_CTXT(ITERSTATE)					\
		)										\
	)_XENUM5_TT_DBG_NODE_NAME_INDEXPATH(							\
		_XENUM5_TT_ITERPOS_INDEXPATH(							\
			_XENUM5_TT_STACKENTRY_ITERPOS(						\
				_XENUM5_TT_STATE_STACKHEAD(ITERSTATE)				\
			)									\
		)										\
	)]
...
#define _XENUM5_TT_FLAT_GEN_ITERATE_CHILDNODES_I1(ITERSTATE, HEAD)				\
	BOOST_PP_CAT(										\
		...
		BOOST_PP_EQUAL(1, BOOST_PP_SEQ_SIZE(_XENUM5_TT_STATE_STACK(ITERSTATE)))		\
	) (ITERSTATE, HEAD)									\
	BOOST_PP_CAT(										\
		...
		BOOST_PP_AND(									\
			BOOST_PP_BOOL(								\
				_XENUM5_TT_ITERPOS_LEVEL(					\
					_XENUM5_TT_STACKENTRY_ITERPOS(HEAD)			\
				)								\
			),									\
			BOOST_PP_NOT(								\
				_XENUM5_TT_ITERPOS_NEXTCHILD(					\
					_XENUM5_TT_STACKENTRY_ITERPOS(HEAD)			\
				)								\
			)									\
		)										\
...
#define _XENUM5_TT_FLAT_GEN_ITERATE_CHILDNODES_1(ITERSTATE, HEAD)				\
	BOOST_PP_REPEAT(									\
		_XENUM5_TT_ITERPOS_CHILDCOUNT(_XENUM5_TT_STACKENTRY_ITERPOS(HEAD)),		\
		_XENUM5_TT_FLAT_GEN_EXEC_CHILDNODE,						\
		ITERSTATE									\
	)
/*
_TT_FLAT_GEN_ITERATE_CHILDNODES_1 _XENUM5_TT_DBG_NODE_NAME(ITERSTATE) node=_XENUM5_TT_STACKENTRY_NODE(HEAD) _XENUM5_NWLN \
*/
...
#define _XENUM5_TT_FLAT_GEN_EXEC_CHILDNODE_I1(ITERSTATE, HEAD, N)				\
	_XENUM5_TT_FLAT_GEN_EXEC_CHILDNODE_I2(							\
		ITERSTATE,									\
		_XENUM5_TT_STACKENTRY_ITERPOS(HEAD),						\
		BOOST_PP_SEQ_ELEM(N, _XENUM5_TT_STACKENTRY_NODE(HEAD)),				\
		_XENUM5_TT_STATE_CTXT(ITERSTATE),						\
		N										\
	)
...
#define _XENUM5_TT_FLAT_GEN_EXEC_CHILDNODE_I2(ITERSTATE, ITERPOS, CHILDNODE, CTXT, N)		\
	_XENUM5_TT_FLAT_GEN_EXEC_CHILDNODE_I3(							\
		ITERSTATE,									\
		_XENUM5_TT_ITERPOS_INIT(							\
			BOOST_PP_DEC(_XENUM5_TT_ITERPOS_LEVEL(ITERPOS)),			\
			BOOST_PP_SEQ_PUSH_BACK(_XENUM5_TT_ITERPOS_INDEXPATH(ITERPOS), N),	\
			0,									\
...
#define _XENUM5_TT_FLAT_GEN_EXEC_CHILDNODE_I3(ITERSTATE, ITERPOS, CHILDNODE, CTXT)		\
	BOOST_PP_CAT(										\
		...
		_XENUM5_TT_STATE_FILTER(ITERSTATE) (ITERPOS, CHILDNODE, CTXT)			\
...
#define _XENUM5_TT_DEPTH_CALC_EXEC(ITERSTATE, HEAD)						\
	...
	(											\
		...
		_XENUM5_TT_STACKENTRY_NODE(HEAD),						\
		_XENUM5_TT_STATE_CTXT(ITERSTATE),						\
		_XENUM5_TT_STATE_CDATA(ITERSTATE)						\
	)
...
#define _XENUM5_TT_DEPTH_CALC_EXEC_I2(ITERSTATE, ITERPOS, NODE, CTXT, CDATA)			\
	BOOST_PP_CAT(										\
		...
		_XENUM5_TT_STATE_FILTER(ITERSTATE) (ITERPOS, NODE, CTXT, CDATA)			\
...
#define _XENUM5_TT_DEPTH_CALC_EXEC_1(ITERSTATE, ITERPOS, NODE, CTXT, CDATA)			\
	_XENUM5_TT_STATE_CALLBACK(ITERSTATE) (ITERPOS, NODE, CTXT, CDATA)
