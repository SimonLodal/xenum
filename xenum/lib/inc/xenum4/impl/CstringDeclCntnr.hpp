 ...
 */
#ifndef _XENUM4_IMPL_CSTRING_DECL_CNTNR_HPP
#define _XENUM4_IMPL_CSTRING_DECL_CNTNR_HPP


// ======================================= FUNCTIONS ============================================
/**
 * Worker for _XENUM4_PROP_DECLC_CSTRING().
 * Declares the functions related to a single custom property, in container class.
 * @hideinitializer
 */
#define _XENUM4_CSTRING_DECLC_FUNCS(PROPNAME, DEPTH, CTXT, Z)					\
public:												NWLN \
	BOOST_PP_REPEAT_ ## Z									\
	(											\
		/* INC() because _Nodes also has indexnodes for the leaf string values */	\
		BOOST_PP_INC(DEPTH),								\
		_XENUM4_CSTRING_DECLC_GET_SIZE,							\
		PROPNAME									\
	)											\
	_XENUM4_CSTRING_DECLC_GET_VALUE(							\
		PROPNAME,									\
		DEPTH,										\
...
/**
 * Callback worker for _XENUM4_CSTRING_DECLC_FUNCS() iteration over levels.
 * Declares get${propname}Size() for this level.
 * @hideinitializer
 */
#define _XENUM4_CSTRING_DECLC_GET_SIZE(Z, LEVEL, PROPNAME)					\
IND1	static const size_t BOOST_PP_CAT(BOOST_PP_CAT(get, PROPNAME), Size) (			\
		_XENUM4_PROP_GEN_INDEX0_PARMS(Enum, size_t, LEVEL, Z)				\
	);											NWLN

/**
 * Worker for _XENUM4_CSTRING_DECLC_FUNCS().
 * Declares get${propname}() value getter.
 * @hideinitializer
 */
#define _XENUM4_CSTRING_DECLC_GET_VALUE(PROPNAME, DEPTH, PROPDEF, Z)				\
...
#endif // _XENUM4_IMPL_CSTRING_DECL_CNTNR_HPP
