...
IND1	const size_t BOOST_PP_CAT(BOOST_PP_CAT(get, PROPNAME), Size) (				\
		_XENUM3_PROP_GEN_INDEX1_PARMS(size_t, LEVEL, Z)					\
	)											\
	...
	{											\
		...
		:: BOOST_PP_CAT(BOOST_PP_CAT(get, PROPNAME), Size) (				\
			value									\
			_XENUM3_PROP_GEN_INDEX1_ARGS(LEVEL, Z)					\
		);										\
	}											NWLN
...
IND1	const _XENUM3_PROPDEF_GET_REAL_TYPE(PROPDEF)*						\
	BOOST_PP_CAT(get, PROPNAME) (								\
		_XENUM3_PROP_GEN_INDEX1_PARMS(size_t, DEPTH, Z)					\
	)											\
	...
	{											\
		...
		:: BOOST_PP_CAT(get, PROPNAME) (						\
			value									\
			_XENUM3_PROP_GEN_INDEX1_ARGS(DEPTH, Z)					\
		);										\
	}											NWLN
