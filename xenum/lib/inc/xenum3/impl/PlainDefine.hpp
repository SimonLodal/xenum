...
#define _XENUM3_PLAIN_DEFINE_DATA(CTXT, PROPDEF, SCOPE, CNTNRNAME, PROPNAME, Z)			\
	...
	constexpr const										\
		...
		[] =										\
		{										NWLN \
			_XENUM3_PLAIN_VALUES_DATA(CTXT)						\
		};										NWLN \
	BOOST_PP_CAT(										\
		_XENUM3_PLAIN_DATA_MULTILEVEL_,							\
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_DATA_MULTILEVEL_0(CTXT, PROPDEF, SCOPE, CNTNRNAME, PROPNAME, Z)
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_DATA_MULTILEVEL_1(CTXT, PROPDEF, SCOPE, CNTNRNAME, PROPNAME, Z)		\
	_XENUM3_PLAIN_VALUES_NAMES(CTXT, PROPDEF, SCOPE, CNTNRNAME, PROPNAME)			\
	_XENUM3_PLAIN_NODES_NAMES(CTXT, PROPDEF, SCOPE, CNTNRNAME, PROPNAME, Z)			\
...
#define _XENUM3_PLAIN_VALUE_DATA(ITERPOS, NODE, CTXT)						\
IND1	BOOST_PP_IF(										\
...
/**
 * Worker for _XENUM3_PLAIN_VALUES_NAMES(); loop function for each data node.
 * Declares a single value name.
 * @hideinitializer
 */
#define _XENUM3_PLAIN_VALUE_NAME(ITERPOS, NODE, CTXT)						\
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_ITER_NODES(CALLBACK, CTXT)						\
	/* First, execute callback only for the enum values (root nodes), so they are */	\
	/* executed in one block. */								\
	_XENUM3_CALL_VALS(									\
		_XENUM3_PLAIN_ITER_NODES_ROOT,							\
		_XENUM3_CTXT_SET_CALLBACK(CTXT, CALLBACK)					\
	)											\
	/* Secondly, iterate all non-root nodes. */						\
	_XENUM3_CALL_VALS(									\
		_XENUM3_PLAIN_ITER_NODES_NONROOT,						\
		_XENUM3_CTXT_SET_CALLBACK(CTXT, CALLBACK)					\
	)

/**
 * Callback worker for _XENUM3_PLAIN_ITER_NODES(); loop function for each root node
 * (enum-value). Execute the callback only for the root node (no further iteration).
 * @hideinitializer
 */
#define _XENUM3_PLAIN_ITER_NODES_ROOT(CTXT, IDENT, ...)						\
	_XENUM3_PLAIN_ITER_NODES_ROOT_I1							\
	(											\
...
/**
 * Worker for _XENUM3_PLAIN_ITER_NODES_ROOT().
 * @hideinitializer
 */
#define _XENUM3_PLAIN_ITER_NODES_ROOT_I1(DATA, DEPTH, CTXT)					\
...
/**
 * Callback worker for _XENUM3_PLAIN_ITER_NODES(); loop function for each node.
 * @hideinitializer
 */
#define _XENUM3_PLAIN_ITER_NODES_NONROOT(CTXT, IDENT, ...)					\
	_XENUM3_PLAIN_ITER_NODES_NONROOT_I1							\
	(											\
		_XENUM3_GET_VARARG(_XENUM3_CTXT_GET_PROPINDEX(CTXT), __VA_ARGS__),		\
...
/**
 * Worker for _XENUM3_PLAIN_ITER_NODES_NONROOT().
 * Execute tupletree iteration.
 * @hideinitializer
 */
#define _XENUM3_PLAIN_ITER_NODES_NONROOT_I1(DATA, DEPTH, CTXT)					\
	_XENUM3_TUPLETREE_ITERATE_FLAT_GEN(							\
		DATA,										\
		/* We do not want to iterate leaf values */					\
		BOOST_PP_DEC(DEPTH),								\
		(_XENUM3_CTXT_GET_CALLBACK(CTXT), _XENUM3_PLAIN_ITER_NODES_NONROOT_FLT),	\
		CTXT										\
	)
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_ITER_NODES_NONROOT_FLT(ITERPOS, NODE, CTXT)				\
	BOOST_PP_AND(										\
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_NODES_NAMES(CTXT, PROPDEF, SCOPE, CNTNRNAME, PROPNAME, Z)			\
	typedef struct										\
	{ NWLN											\
		_XENUM3_PLAIN_ITER_NODES(_XENUM3_PLAIN_NODE_NAME, CTXT)				\
	} BOOST_PP_CAT(PROPNAME, _NodeNames_t);							\
	NWLN

/**
 * Worker for _XENUM3_PLAIN_NODES_NAMES().
 * @hideinitializer
 */
#define _XENUM3_PLAIN_NODE_NAME(ITERPOS, NODE, CTXT)						\
IND1	_XENUM3_DECL_GET_SCOPE(_XENUM3_CTXT_GET_DECL(CTXT))					\
	_XENUM3_DECL_GET_CNTNRNAME(_XENUM3_CTXT_GET_DECL(CTXT))					\
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_NODES_DATA(CTXT, PROPDEF, SCOPE, CNTNRNAME, PROPNAME, Z)			\
	constexpr const										\
		...
		[] =										\
	{ NWLN											\
		_XENUM3_PLAIN_ITER_NODES(_XENUM3_PLAIN_NODE_DATA, CTXT)				\
	};											\
	NWLN											\
	static_assert(										\
...
/**
 * Worker for _XENUM3_PLAIN_NODES_DATA().
 * @hideinitializer
 */
#define _XENUM3_PLAIN_NODE_DATA(ITERPOS, NODE, CTXT)						\
IND1	{											\
		/* Size */									\
		_XENUM3_TUPLETREE_ITERPOS_GET_CHILDCOUNT(ITERPOS),				\
		/* Index */									\
		BOOST_PP_CAT(									\
			_XENUM3_PLAIN_NODE_DATA_INDEX_,						\
			BOOST_PP_BOOL(_XENUM3_TUPLETREE_ITERPOS_GET_CHILDCOUNT(ITERPOS))	\
		) (ITERPOS, CTXT)								\
	},											\
	NWLN
/*
_XENUM3_PLAIN_NODE_DATA: iterpos={_XENUM3_TUPLETREE_ITERPOS_DUMP(ITERPOS)} ctxt=[CTXT] node=[NODE] NWLN \
*/

/**
 * Worker for _XENUM3_PLAIN_NODE_DATA().
 * Define IndexNode.index to 0 since node has no children.
 * @hideinitializer
 */
#define _XENUM3_PLAIN_NODE_DATA_INDEX_0(ITERPOS, CTXT)						\
	0
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_NODE_DATA_INDEX_1(ITERPOS, CTXT)						\
	_XENUM3_PLAIN_NODE_DATA_INDEX_1_DO(							\
		CTXT,										\
...
/**
 * Worker for _XENUM3_PLAIN_NODE_VALUE_INDEX_1().
 * @hideinitializer
 */
 ...
 * @hideinitializer
 */
#define _XENUM3_PLAIN_DEFINE_DEBUG(CTXT, PROPDEF, Z)						\
	_XENUM3_PLAIN_DEFINE_FUNC_DUMP_NODES(CTXT, PROPDEF, _XENUM3_PROPDEF_GET_NAME(PROPDEF))	\

/**
 * Worker for _XENUM3_PLAIN_DEFINE_DEBUG().
 * @hideinitializer
 */
#define _XENUM3_PLAIN_DEFINE_FUNC_DUMP_NODES(CTXT, PROPDEF, PROPNAME)				\
